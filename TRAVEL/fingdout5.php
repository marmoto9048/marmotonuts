<!-- 数据库查询 -->
      <title>路线检索！</title>

    <body>
        <form action="" method="post" name="myForm" >
             <div class="form-group" style="width: 432px;">
                  <h3 style="width: 170px;
color:#454545;">请输入要检索路线的title信息： </h3>
   
<input type="text"  name="keyword" id="div">
            </div>

 <input class="btn btn-primary btn-block" type="submit"  value="提交" name="submit">

       </form>
<script>
function validateForm(){
              
          var  formElement = document.myForm;
          var keyword = formElement.keyword.value;

             keyword = trim(keyword);
             checkkeyword(keyword);
         }
     
         function trim(s){
             s = s.replace(/(^\s*)|(\s*$)/g, "");//
             return s;
         }
         window.onload = function(){
             var formElement = document.myForm;
             formElement.onsubmit=validateForm;
         }; 
</script>  



</body></html>
<?php
ob_start();
 $_GET['keyword'] = empty($_GET['keyword']) ? '' : $_GET['keyword'];
 $keyword=$_REQUEST["keyword"];
?>

<?php
// $keyword=玩;
ob_start();
//假设用户登录成功获得了以下用户数据
$userinfo = array('keyword' => $keyword,);
echo "keyword11:   ";
print_r($keyword);
echo "<br>";
// =================================================================
@ $db = mysqli_connect('localhost','root','');  
mysqli_query($db,"set names utf8;");  
mysqli_select_db($db,'travel');//找到数据库mapply  
if(mysqli_connect_errno()){  
 echo "Error:Could not connect to mysqli database.";  
 exit;  
}  
echo '数据库连接成功    '.'<br>';
echo 'keyword22为：      ';
echo $keyword.'<br>'.'--------------------'.'<br>'; 
echo '$keyword33: ';
echo $keyword.'<br>';
$q="SELECT count(*) FROM apply where title like  '%".$keyword."%'";
echo 'q: '.$q.'<br>';
$k="SELECT * FROM apply where title like '%".$keyword."%'";//对表apply进行选择  
echo $k;




$result1 = mysqli_query($db,$q);  //执行
$row1 = mysqli_fetch_array($result1);  //得到有几条结果
$allnum=$row1[0];  //


$result = mysqli_query($db,$k);//执行$k的mysql语句，并赋给result  
$rownum = mysqli_num_rows($result);//获取result的数据数量  
echo '共有'.$rownum.'条数据'; 
$apply=array(array()); 
?>
<div style="width: 80%;padding-left: 20%;">
 <table  style="width: 100%"; border="18"; "    cellspacing="0" cellpadding="0" border="1">
            <tr>
                <th>申请id:</th>
                <th>标题</th>
                <th>出行计划</th>
                <th>众筹人数</th>
                <th>发布时间</th>
                <th>点赞人数</th>
            </tr>
            <?php
                //获取表中的数据
                while($row=mysqli_fetch_assoc($result)){
            ?>
            <tr>
                <td><?php echo $row['id'];?></td>
                <td><a href="article.php?title=<?php echo $row['title'];?>"><?php echo $row['title'];?></a></td>
                <td><?php echo $row['path1'];?></td>
                <td><?php echo $row['people'];?></td>
                <td><?php echo $row['dayyy'];?></td>
                <td><?php echo $row['thumbs_up'];?></td>
                
               
            </tr>
            <?php
                }
            ?>
        </table>
</div>
</body>
 </html>